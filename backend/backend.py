...
def startup():
    try:
        ...
        # Pool de conexiones: min 1, max 10 (ajustar según necesidad)
...
def _get_conn_from_pool():
    db_pool = getattr(app.state, "db_pool", None)
    if db_pool is None:
        raise HTTPException(status_code=500, detail="Conexión a la base de datos no disponible")
...
def register_user(data: Register):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO usuarios (nombre, apellido, correo, contrasena) VALUES (%s,%s,%s,%s) RETURNING id",
...
def login_user(data: Login):
    ...
    try:
        ...
        cur.execute("SELECT id, nombre, apellido, correo, contrasena FROM usuarios WHERE correo = %s", (data.correo,))
        user = cur.fetchone()
        if not user:
            raise HTTPException(status_code=401, detail="Correo o contraseña incorrectos")

        stored_hash = user.get("contrasena")
        if not stored_hash or not bcrypt.checkpw(data.contrasena.encode("utf-8"), stored_hash.encode("utf-8")):
            raise HTTPException(status_code=401, detail="Correo o contraseña incorrectos")

        return {
            "mensaje": "Login exitoso",
            "usuario": {"id": user["id"], "nombre": user["nombre"], "apellido": user["apellido"], "correo": user["correo"]}
        }
    except HTTPException:
        raise
    except Exception as e:
